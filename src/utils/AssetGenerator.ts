...
export class AssetGenerator {
  ...
  static generateMario(scene: Phaser.Scene) {
    const textures = [
      ...
      // Big Mario - full action variants
      ...
      { key: 'mario_big_death', w: 32, h: 64 },
      { key: 'mario_big_flag', w: 32, h: 64 },
      { key: 'mario_big_throw', w: 32, h: 64 },
      { key: 'mario_big_slide', w: 32, h: 64 },
      // Fire Mario - full action variants
      { key: 'mario_fire_idle', w: 32, h: 64 },
      { key: 'mario_fire_run1', w: 32, h: 64 },
      { key: 'mario_fire_run2', w: 32, h: 64 },
      { key: 'mario_fire_run3', w: 32, h: 64 },
      { key: 'mario_fire_jump', w: 32, h: 64 },
      { key: 'mario_fire_crouch', w: 32, h: 64 },
      { key: 'mario_fire_death', w: 32, h: 64 },
      { key: 'mario_fire_flag', w: 32, h: 64 },
      { key: 'mario_fire_throw', w: 32, h: 64 },
      { key: 'mario_fire_slide', w: 32, h: 64 }
    ];
  ...
  static drawBigMario(g: Phaser.GameObjects.Graphics, key: string) {
    ...
    const isJump = key.includes('jump');
    const isRun = key.includes('run');
    const isDeath = key.includes('death');
    const isFlag = key.includes('flag');
    const isThrow = key.includes('throw');
    const isSlide = key.includes('slide');
    const frame = isRun ? parseInt(key.slice(-1)) : 1;
    const shoeColor = COLORS.marioBrown;
  ...
  static drawFireMario(g: Phaser.GameObjects.Graphics, key: string) {
    ...
    if (isCrouch) {
      ...
      g.fillRect(8 * px, startY + 13 * px, 2 * px, 2 * px);
      g.fillRect(11 * px, startY + 13 * px, 2 * px, 2 * px);
      ...
      g.fillRect(3 * px, 5 * px, 14 * px, 6 * px);
      g.fillStyle(0x000000);
      g.fillRect(6 * px, 6 * px, 3 * px, 2 * px);
      g.fillRect(11 * px, 6 * px, 3 * px, 2 * px);
      g.fillRect(8 * px, 9 * px, 4 * px, 1 * px);

      g.fillStyle(COLORS.fireFlower);
      g.fillRect(2 * px, 11 * px, 16 * px, 10 * px);

      g.fillStyle(COLORS.fireFlower);
      g.fillRect(4 * px, 22 * px, 5 * px, 4 * px);
      g.fillRect(11 * px, 22 * px, 5 * px, 4 * px);

      g.fillStyle(shoeColor);
      g.fillRect(3 * px, 26 * px, shoeW * px, shoeH * px);
      g.fillRect(12 * px, 26 * px, shoeW * px, shoeH * px);
    } else {
      // 帽子
      g.fillStyle(COLORS.fireFlower);
      g.fillRect(5 * px, 0, 10 * px, 3 * px);
